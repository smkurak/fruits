...
################################################################################
# Шрифты
style pacifico_title:
    font "fonts/Pacifico-Regular.ttf"
    size 48
    color "#FFBB5C"
    outlines [(2, "#4A2C00", 0, 0)]
...
style map_button is button:
    xminimum 280
    xalign 0.5
    background Frame("#FFBB5C99", 10, 10)
    hover_background Frame("#FFBB5CFF", 10, 10)
    padding (20, 10)

style map_button_text is button_text:
    color "#FFFFFF"
    hover_color "#4A2C00"
    xalign 0.5
    font "fonts/Nunito-Regular.ttf"

style map_button_disabled is button:
    xminimum 280
    xalign 0.5
    background Frame("#88888866", 10, 10)
    padding (20, 10)
    font "fonts/Nunito-Regular.ttf"

style map_button_disabled_text is button_text:
    color "#AAAAAA"
    xalign 0.5
    font "fonts/Nunito-Regular.ttf"
...
screen main_menu():
    ...
    on "show" action Play("music", "audio/music/main_menu_01.mp3", loop=True, fadein=2.0)

    add "bg/main_menu_bg.jpg"

    # add Movie(play = "gui/main_menu_02.ogv", loop = True)

    # с заглушкой 
    # add Movie(
    #     play="gui/main_menu_02.ogv",
    #     loop=True,
    #     image="bg/main_menu_bg.jpg"  # показывается пока видео не началось
    # )
    ...
    frame:
        ...
        vbox:
            spacing 15
            xalign 0.5
            textbutton "Начать"      action Start()
...
screen intro_screen():
    ...
    frame:
        ...
        vbox:
            ...
            text "Добро пожаловать на Остров Фруктового Сада" style "pacifico_title"

            null height 10

            text "Ты назначен новым Смотрителем магического острова, где каждый фрукт давно обрёл душу и облик прекрасной девушки." style "nunito"

            text "Но сад начинает чахнуть. Девушки замкнулись в себе — они не могут дозреть без чьей-то заботы и тепла." style "nunito"

            text "Твоя задача — найти подход к каждой из них, завоевать доверие и помочь им раскрыться." style "nunito"

            null height 10

            text "У тебя есть 7 дней. Каждый день можно посетить 1–2 девушки." style "nunito"

            null height 20

            textbutton "Начать" action Return() xalign 0.5 style "map_button"
...
screen island_map():
    ...
    on "show" action Play("music", "audio/music/island_map.mp3", loop=True, fadein=2.0)

    add "bg/island_map.jpg"

    # Счётчик дня и визитов — верхний левый угол
    frame:
        xalign 0.0
        yalign 0.0
        xoffset 20
