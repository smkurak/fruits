...
# ----------------------------------------
# ПОДАРОК (ракушка, найденная на прогулке — game/exploration.rpy)
# ----------------------------------------

label nana_gift_offer:
    scene bg nana with fade
    show nana at center with dissolve

    n "Погоди-ка. У тебя ракушка. Хорошая, между прочим."

    menu:
        "Это тебе.":
            $ use_favorite_item("nana")
            n "...Мне? Серьёзно? Ладно, давай сюда, пока не передумал."
            $ renpy.notify("🍍 Нана рада подарку!")

        "Так, ничего особенного.":
            n "Хм. Ладно, не мой день."
...
label nana_stage1:
    ...
    if _first_visit:
        ...
        "Ты подходишь ближе. Песок хрустит под ногами — нарочно громко, чтобы она услышала."

        n "Хм. Даже подкрасться не пытаешься. Ладно, это хотя бы честно."
    else:
    ...
    menu:
        "Я здесь, чтобы помочь. Всем вам." if choice_open("nana", 1, 0):
            ...
            n "Обычно после таких слов пропадают через день. Ты не такой особенный, чтобы стать исключением."

        "Ты выглядишь… необычно." if choice_open("nana", 1, 1):
            $ use_choice("nana", 1, 1)
            $ add_affection("nana", 8)
            n "Ха. Прямолинейный. Хотя бы не врёшь."
            n "Большинство либо пялятся молча, либо делают вид, что не заметили. Ты хотя бы сказал вслух."

        "Я могу уйти, если мешаю." if choice_open("nana", 1, 2):
            $ use_choice("nana", 1, 2)
            $ add_affection("nana", 3)
            n "...Оставайся. Всё равно скучно одной."
            n "Только не жди, что я буду развлекать гостя. Стой, где стоишь, если хочешь."

        "Могла бы быть и повежливее." if choice_open("nana", 1, 3):
            $ use_choice("nana", 1, 3)
            $ add_affection("nana", -3)
            n "Повежливее? Может, ещё чаю с печеньем подать?"
            n "Вежливость — это для тех, кто заслужил. Ты пока не в том списке."

    if _first_visit:
        n "Слушай сюда, 'смотритель'. Я не фрукт, который сам падает в руки."
        "Она наконец поворачивается — ровно настолько, чтобы посмотреть на тебя, прежде чем снова отвернуться к морю."
...
label nana_stage2:
    ...
    if _first_visit:
        ...
        "Она молча кивает на место рядом с собой — впервые не прогоняя, а приглашая."

        n "Только не подумай, что это что-то значит. Просто песок нагрелся с двух сторон, вот и всё."
    else:
    ...
    menu:
        ...
        "Признай, ты уже скучаешь по мне, когда меня нет." if choice_open("nana", 2, 3):
            ...
            n "...Хотя было тише, когда тебя не было. Не знаю, хорошо это или нет."

    if _first_visit:
        n "Твои старания заметны... Но не думай, что я сразу позволю себя трогать."
        "Она смотрит на море, а не на тебя, но подвигается чуть ближе, пока говорит."
...
label nana_stage3:
    ...
    if _first_visit:
        ...
        "Она тут же отворачивается, будто сказала лишнее."

        n "...Не обращай внимания. Просто вслух подумала."
    else:
    ...
    menu:
        ...
        "Может, хватит уже играть в недотрогу?" if choice_open("nana", 3, 3):
            ...
            n "Ты почти дошёл. Не порти всё спешкой."

    if _first_visit:
        n "...Не уходи сразу. Ещё немного побудь здесь."
        "Она садится настолько близко, что вы почти соприкасаетесь плечами — и делает вид, что не заметила этого."
...
label nana_stage4:
    ...
    if _first_visit:
        ...
        "Она подходит вплотную — в первый раз без всякой дистанции между вами."

        n "Не жди, что это будет длинная речь. Просто... останься."
    else:
